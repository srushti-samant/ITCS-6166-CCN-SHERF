...
def subprocess_fn(rank, c, temp_dir):
    ...
    if c.num_gpus > 1:
        ...
        if os.name == 'nt':
           ...
           # Pass the timeout as an integer (e.g., 600 seconds)
            torch.distributed.init_process_group(backend='gloo', init_method=init_method, rank=rank, world_size=c.num_gpus, timeout=timeout)
...
def launch_training(c, desc, outdir, dry_run):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if c.num_gpus == 1:
            ...
            torch.multiprocessing.spawn(fn=subprocess_fn, args=(c, temp_dir), nprocs= 1)
...
def main(**kwargs):
    ...
    if opts.cfg == 'RenderPeople':
        ...
        dataset_kwargs = dnnlib.EasyDict(class_name='training.NeuBody_dataset.NeuBodyDatasetBatch', data_root=opts.data, split='train', num_instance=6, poses_start=0, poses_interval=5, poses_num=100, image_scaling=opts.neural_rendering_resolution_initial/1024, sample_obs_view=opts.sample_obs_view, fix_obs_view=opts.fix_obs_view)
        # dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs) # Subclass of training.dataset.Dataset.
        c.training_set_kwargs = dataset_kwargs
        c.training_set_kwargs.resolution = 512
        dataset_name = opts.cfg
